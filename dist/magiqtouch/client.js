...
import { all, expectHeaterFanSpeed, expectHeaterSetTemp, expectMode, expectSystemOn, } from './checker.js';
import { fetchSystemDetails } from './rest.js';
import { MagIQTouchWebSocket } from './websocket.js';
import { RUNNING_STATE_IDLE, RUNNING_STATE_REQUIRED, getHeater, } from './models.js';
...
export class MagIQTouchClient extends EventEmitter {
    ...
    async start() {
        ...
        this.ws = new MagIQTouchWebSocket(() => this.auth.getIdToken(), macAddress, this.log, this.wsOptions);
        this.ws.on('status', (status) => {
            ...
            try {
                ...
                this.log.debug(`status listener error: ${err.message}`);
            }
        });
    ...
    async refreshStatus() {
        ...
        try {
            ...
            this.log.debug(`status refresh failed: ${err.message}`);
            return undefined;
        }
    }
    async stop() {
    ...
    async command(mutate, checker) {
        ...
        const params = structuredClone(this.lastStatus);
        mutate(params);
        ...
        try {
            ...
            this.log.warn(`command not confirmed: ${err.message}; resyncing`);
            await this.refreshStatus();
            throw err;
        }
    }
