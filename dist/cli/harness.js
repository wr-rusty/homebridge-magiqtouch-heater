...
function parseArgs(argv) {
    const args = { on: false, off: false, fanOnly: false };
...
async function main() {
    ...
    try {
        ...
        console.log('Heater temp: ', `${sys.Heater?.MinimumTemperature ?? '?'}–${sys.Heater?.MaximumTemperature ?? '?'}`);
        ...
        if (MagIQTouchClient.isAuthError(err)) {
            console.error('\nAUTH FAILED — check MQT_EMAIL / MQT_PASSWORD:', err.message);
        }
        else {
            console.error('\nError:', err);
        }
