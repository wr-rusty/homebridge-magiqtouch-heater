...
export async function fetchSystemDetails(
  auth: CognitoAuth,
  log: Logger,
): Promise<{ system: SystemDetails; macAddress: string }> {
  ...
  try {
    ...
    throw new Error(`devices/system returned an unparseable body: ${(err as Error).message}`);
  }
  if (!Array.isArray(body) || body.length === 0) {
    throw new Error(`devices/system returned no systems: ${JSON.stringify(body)}`);
  }

  const system = body[0] as SystemDetails;
