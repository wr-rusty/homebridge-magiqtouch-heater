...
export interface SystemDetails {
  System?: { Name?: string; Address?: string };
  Wifi_Module?: { MacAddressId?: string; version?: string; type?: string };
  Heater?: {
    InSystem?: boolean;
  ...
  ACZones?: { Zones?: Array<{ Name: string; Type: string }> };
  NoOfZoneControls?: number;
  WallController?: { Firmware?: string; Type?: number };
}
 ...
 */
export function getHeater(status: RemoteStatus | undefined): UnitDetails | undefined {
  return status?.heater?.[0];
}

/** True if the heater unit is actively firing (not merely powered on). */
export function isHeaterRunning(unit: UnitDetails | undefined): boolean {
 ...
 */
export function unitTempToCelsius(value: number, units: TemperatureUnit): number {
...
export function celsiusToUnitTemp(celsius: number, units: TemperatureUnit): number {
  if (units && units.toLowerCase() === 'f') {
